...
- [ ] **Still the one real open item**: the actual LG webOS SSAP pairing prompt confirmed accepted on the TV screen, end-to-end from Hearth's UI. Every prerequisite is now verified working multiple times over (relay, TLS trust, pairing, reconnection) — this is purely a "get eyes on the real device" checkpoint at this point, not a code gap.
