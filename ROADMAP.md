...
- [x] **Family Command Center side built (2026-09-11)** — no longer out of reach once SSH access to the Pi became an established capability mid-session. Real finding: the Pi's kiosk runs under **labwc (Wayland)**, not X11 — `x11vnc` (also installed) can't work here at all; **`wayvnc`** (also already installed, bundled with unused Raspberry Pi Connect, left untouched) is the correct tool. Built `hearth-vnc-server.service` (dedicated wayvnc, `--websocket` mode, loopback `127.0.0.1:5901`) + `hearth-relay-vnc.service` (new standalone relay, `scripts/hearth-relay/vnc-relay-ws-server.js`, same bearer-token auth as the Samsung/LG relay but a separate file — that relay forbids targeting the Pi itself by design, this one's only valid target *is* the Pi). Corrected Hearth's `familyCommandCenterVncRelay.ts` to point at this dedicated port (3212) instead of a Next.js route path, matching the same "`next start` can't expose WS upgrades" constraint the Samsung/LG relay already worked around. **Verified live end-to-end** — a real RFB handshake received through the full chain — without ever sending an input event, so the live kiosk display was never touched.
- [ ] **Sean, real-hardware checkpoint** — pair, open the trackpad/keyboard screen, confirm the Pi's actual cursor moves and keystrokes land. The one thing left that only Sean can do: send a real input event and watch it land.
- [ ] Per-device VNC-relay permission grant/revoke ("multiple phones, with permission") — not built yet; today any device holding a valid paired FCC token can reach the relay, an all-or-nothing grant rather than per-device.
...
- [x] **Reliability parity check across all four drivers, 2026-09-10** — confirmed by reading, not assumed: Samsung had the identical "retry only after a successful connect drops" gap LG had, now fixed the same way (178/178 tests). Sony and Roku's stateless-HTTP architecture already retried a failed connect() correctly from the start — nothing to fix there. MAC-based re-discovery ported to Samsung too; not yet ported to Sony/Roku (different transport, no natural hook for the same distinction) — worth its own pass if their real-hardware checkpoints ever surface a stale-IP case.
